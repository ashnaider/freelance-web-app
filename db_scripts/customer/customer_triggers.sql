...
DROP FUNCTION IF EXISTS DELETE_NEW_APPLICATIONS_WHEN_JOB_BLOCKED_F();
...
DROP TRIGGER IF EXISTS UNLOCK_CUSTOMER on customer;
DROP FUNCTION IF EXISTS UNLOCK_CUSTOMER_F();
...
CREATE OR REPLACE FUNCTION UNLOCK_CUSTOMER_F() RETURNS TRIGGER
    LANGUAGE PLPGSQL
AS $$
    BEGIN
...
CREATE TRIGGER UNLOCK_CUSTOMER
    BEFORE UPDATE ON customer
    FOR EACH ROW
    EXECUTE PROCEDURE UNLOCK_CUSTOMER_F();
...
CREATE OR REPLACE FUNCTION DELETE_NEW_APPLICATIONS_WHEN_JOB_BLOCKED_F() RETURNS TRIGGER
    LANGUAGE PLPGSQL
AS $$
    BEGIN
        if new.is_blocked = true then
            delete from application where job_id = new.id and status = 'new';
        end if;
...
CREATE TRIGGER DELETE_NEW_APPLICATIONS_WHEN_JOB_BLOCKED
    BEFORE UPDATE ON new_job
    FOR EACH ROW
    EXECUTE PROCEDURE DELETE_NEW_APPLICATIONS_WHEN_JOB_BLOCKED_F();
